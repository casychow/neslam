"""
Field for NeSLAM adds scene contraction and
image embeddings to instant ngp (like nerfacto_field.py)

- Changed num_layers in __init__() to 4 because we need at least
3 to find and extract the fine, coarse, and medium details of
each image if we are to recreate them.

- If increasing num_layers doesn't work, will increasing/decreasing
dimension of hidden layers work (better)?
--- confused with self.mlp_base, self.mlp_head, and self.transient

[] - Edit calling neslam_field to be like calling nerfw_field
[] - Check how nice-slam detects different detail types for MLP

If NICE-SLAM works better, it's probably because of Gaussian Fourier
Feature Transforms: https://github.com/cvg/nice-slam/blob/master/src/conv_onet/models/decoder.py
and https://github.com/ciglenecki/nerf-research (mip-nerf)
- We should add mip-nerf to the mix


stopped using this because i looked at models/mipnerf.py
"""
...
class TCNNNeSLAMField(Field):
    ...
    def __init__(
        ...
        aabb,
        num_images: int,
        num_layers: int = 3, #nerf already has a "fine" and "coarse" detail layer
        # now we want to add "medium" detail layer
        hidden_dim: int = 64,
        geo_feat_dim: int = 15,
...
field_implementation_to_class = {"tcnn": TCNNNeSLAMField, "torch": TorchNeSLAMField}